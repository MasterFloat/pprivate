		...
		remind: function (tournament, user) {
			...
			for (var u in users) {
				...
				if (!targetUser) {
					...
					targetUser.popup('Tienes una batalla del torneo en la "' + tournament.room.title + '". Si no empiezas pronto podrás ser descalificado. / Vous avez un combat dans le tournoi de la salle "' + tournament.room.title + '". Si vous ne le commencer pas bientôt, vous pourriez être disqualifié.');
				}
			}
			tournament.room.addRaw('<b>Los jugadores han sido recordados de sus batallas en el torneo por ' + user.name + '. / Les joueurs ont ete rappele de leurs combats du tournoi par ' + user.name + '.</b>');
			if (offlineUsers.length > 0 && offlineUsers !== '') tournament.room.addRaw('<b>Los siguientes usuarios estan offline ahora mismo / Les utilisateurs suivants ne sont plus en ligne: ' + offlineUsers + '.</b>');
		},
		scout: 'setscouting',
